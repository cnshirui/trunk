...
ss.Game.prototype.move_ = function() {
  for (var i = 0; i < this.spriteCounter; i++) {
    if (this.sprites[i].alive) {
      ...
      this.sprites[i].sizeX -= 0.05;
      this.sprites[i].sizeY -= 0.05;
 ...
 */
ss.Game.prototype.addSprite = function(name) {
  ...
  this.sprites[this.spriteCounter] = new ss.Sprite(x, y, speedX, speedY,
      sizeX, sizeY, name, this.spriteDivPrefix_ + this.spriteCounter);
  this.spriteCounter++;
};
 ...
 * @constructor
 */
ss.Sprite = function(x, y, speedX, speedY, sizeX, sizeY, sName, sId) {
  this.x = x;
  this.y = y;
  this.speedX = speedX;
  this.speedY = speedY;
...
ss.zap = function(e) {
  if (e) {
    ...
    killed.push(ss.game.sprites[eventTargetNum].name);
