...
public class AstFloat implements IAstLiteral{
	private Float value;
	private int line;
	
	public AstFloat(float val, int line){
	...
	public void generateCode(DoAction actionTag,SymbolTable st){
		...
		floatVal.setFloat(value.floatValue());
		Push push = new Push();
		push.addValue(floatVal);
		Push.StackValue oneVal = new Push.StackValue();
		oneVal.setInteger(1);
		push.addValue(oneVal);
		Push.StackValue floatClassVal = new Push.StackValue();
		floatClassVal.setString("Float");
		push.addValue(floatClassVal);
