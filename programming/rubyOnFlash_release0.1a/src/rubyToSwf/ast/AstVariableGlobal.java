...
public class AstVariableGlobal implements IAstExpression{
	
	private String varName;
	private int line;
	
	public AstVariableGlobal(String var, int line){
		this.varName = var;
		this.line = line;
	}
	
	public int getType(){
		return IAstNode.VARIABLE_GLOBAL;
	}
	...
	public void generateCode(DoAction actionTag,SymbolTable st){
		Push pushGlobal = new Push();
		Push.StackValue globalVal = new Push.StackValue();
		globalVal.setString("_global");
		pushGlobal.addValue(globalVal);
		actionTag.addAction(pushGlobal);
