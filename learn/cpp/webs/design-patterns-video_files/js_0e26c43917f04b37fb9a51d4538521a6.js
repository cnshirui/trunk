...
function loadCart() {
  ...
  cached = readCookie('cart');
  if (cached) {
    $('.head-center').html(cached);
  }
  ...
  $.ajax({
    ...
    success: function(msg){
      if ((args[0] != 'cart') && (msg != cached)) {
        $('.head-center').html(msg);
        createCookie('cart', msg, 14);
      }
      //setTimeout("loadCart();", 30000);
    }
  });
}
  ...
  if (!$.browser.msie || (parseInt(jQuery.browser.version) != 6 && parseInt(jQuery.browser.version) != 7)) { 
    ...
    $('.nav > li').mouseover(function(){
      if (!$(this).is('.active-trail')) {
        $(this).children('.li-wrapper').removeClass('tohide');
      }
    });
    $('.nav > li').mouseout(function(){
      if (!$(this).is('.active-trail')) {
        $(this).children('.li-wrapper').addClass('tohide');
        if ($.browser.msie) {
          setTimeout("$('.tohide').hide();",50);
        }
        else {
          setTimeout("$('.tohide').fadeOut(200);",50);
        }
      }
    });
  }
...
$(document).ready(function(){
  ...
  $('.login-form input').blur(function(){
    alertTimerId = setTimeout("hide_login_form();", 1000);
  });

  $('.login-link a').click(function(){
    if (!$(this).is(':animated')) {
      $('.login-form').css('display', 'block');
      wid = $('.login-link a').width();
      $('.login-link a').animate({width:1}, 'slow');
      
      $('.login-form').animate({top:-54}, 'slow', function(){
        $('.login-form').css('z-index', 6);
        $('.login-form input[name=name]').focus();
      });
    }
    return false;
  });*/

  if (!$.browser.firefox && !$.browser.safari && !$.browser.mozilla) {
    $('.book-navigation .page-links').corner();
    $('.gray').corner();
  }
  
  
  // Checkout improvements
  label = $('#edit-panes-customer-primary-email-wrapper label');
  input = $('#edit-panes-customer-primary-email-wrapper input');
  $('#edit-panes-customer-primary-email-wrapper').append('<table><tr><td class="field-label tablab" style="padding:2px;"></td><td class="tabinp" style="padding:2px;"></td></tr></table>');
  $('.tablab').append($('.form-required', label));
  $('.tablab').append(label.text());
  label.remove();
  $('.tabinp').append(input);
  $('.tablab').width($('.address-pane-table table td').width());
  ...
  if (!$.browser.msie || (parseInt(jQuery.browser.version) != 6 && parseInt(jQuery.browser.version) != 7)) { 
    $(window).resize(function(){
      if ($('.nav').length) {
        width = $(window).width();
        if (width < 1024) {
          $('.nav .active-trail .menu').css('padding-top', ($('.nav > .last').offset().top - $('.nav .active-trail').offset().top + 5) + 'px');
        }
        else {
          $('.nav .active-trail .menu').css('padding-top', '5px');
        }
      }
    });
...
this.label2value = function(form){	
