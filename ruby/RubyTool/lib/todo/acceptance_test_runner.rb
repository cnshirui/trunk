...
def c_b(var, default = nil) SeleniumOnRailsConfig.get(var, default) end
...
SERVER_COMMAND =      c_b :server_command do
  ...
  if RUBY_PLATFORM =~ /mswin/
    "ruby webrick -p %d -e test > NUL 2>&1"   
  else
    "webrick -p %d -e test"
  ...
  class AcceptanceTestRunner
    ...
    def do_start_server
      ...
      while true
        ...
        raise "server returned error: #" if r.instance_of? Fixnum
    ...
    def start_browser browser, path
      ...
      command = "\"#\" \"http://#:#{@port}#?test=tests&auto=true&baseUrl=#&resultsUrl=postResults/#&multiWindow=#\""
    ...
    def start_subprocess command
      ...
      if RUBY_PLATFORM =~ /mswin/
        ...
        SeleniumOnRails::AcceptanceTestRunner::JavaSubProcess.new command
...
class SeleniumOnRails::AcceptanceTestRunner::SubProcess
  ...
  def stop what
    ...
    begin
      ...
      ##such as the process is already closed (tabbed browser)
    end
  end
end
class SeleniumOnRails::AcceptanceTestRunner::Win32SubProcess < SeleniumOnRails::AcceptanceTestRunner::SubProcess
  def initialize command
    require 'win32/open3'
    # #win32-open3 http://raa.ruby-lang.org/project/win32-open3/
    puts command
    input, output, error, @pid = Open4.popen4 command, 't', true
  end
end

class SeleniumOnRails::AcceptanceTestRunner::JavaSubProcess < SeleniumOnRails::AcceptanceTestRunner::SubProcess
  def initialize command
    puts command
