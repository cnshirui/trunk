...
module JXML
  ...
  # add an error class to show errors with JXML
  class JXMLException < Exception; end
  ...
  class Node
    ...
    def find xpath, range = :all
      ...
      rescue javax.xml.xpath.XPathExpressionException
        raise JXMLException, "Wrong XPath: #{xpath}"
      end
    end
    ...
    #
    def value= new_value
      @instance.setTextContent new_value.to_s
    end
    ...
    #
    # return child index in it's parent, from 0
    #
    def child_index
      parent_node = parent
      if(parent_node.hash == @instance.getOwnerDocument.hashCode)
        return -1
      else
        i = 0
    ...
    def append_child new_child
      if(new_child.kind_of?(String))
        new_child = Element.create(@instance.getOwnerDocument.createElement(new_child))
      end
    ...
    #
    # insert child before position index
    # such as [a, b, c].insert_before(1, d) => [a, d, b, c]
    #
    def insert_before index, new_child
      
      if(index < 0 || index > children.length)
        raise JXMLException, "out of bound: #{index} must in #{(0..children.length).to_a}" 
      end

      # if the deleted node lies in the tail
      if(index == children.length)
        append_child new_child
      else
        index_child = children[index]

        if(new_child.kind_of?(String))
          new_child = Element.create(@instance.getOwnerDocument.createElement(new_child))
        end

        @instance.insertBefore new_child.instance, index_child.instance
      end
    ...
    #
    # colne a node, default deep clone
    #
    def copy(deep = true)
  ...
  class Document < Node
    ...
    def initialize xml
      ...
      rescue org.xml.sax.SAXParseException => e
#        org.xml.sax.SAXParseException
#        raise JXMLException, e.
#            "Content is not allowed in trailing section or not well-formed: #{xml[0..50]}"

#        raise JXMLException, [e.message].concat(e.backtrace).join("\n\t")
         raise JXMLException, e.message, e.backtrace
      end
    end
  ...
  class Element < Node
    ...
    def set_attribute name, value
      @instance.setAttribute name, value.to_s
    end
  end
