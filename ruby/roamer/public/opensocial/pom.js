...
PilesOfMoney.prototype.draw = function(data, options) {
  ...
  for (var i = 0; i < bars.length; i++) {
    ...
    var w = 120;
    var img = 'dollar_1';
    if (h <= 114) { img = 'dollar_2'; w = 121; }
    if (h <= 93)  { img = 'dollar_3'; w = 86; }
    if (h <= 73)  { img = 'dollar_4'; w = 72; }
    if (h <= 56)  { img = 'dollar_5'; w = 61; }
    if (h <= 44)  { img = 'dollar_6'; w = 50; }
    if (h <= 35)  { img = 'dollar_7'; w = 44; }
    if (h <= 29)  { img = 'dollar_8'; w = 39; }
    ...
    html.push('<br />');
    html.push('<img src="http://visapi-gadgets.googlecode.com/svn/trunk/image/', img, '.png" width="', w, '" height="', h, '" />');
    html.push('</td>');
  }
